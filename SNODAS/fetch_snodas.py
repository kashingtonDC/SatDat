...
def clip_tifs(tifs,dst_dir, shapefile = "/Users/aakash/Desktop/SatDat/SNODAS/shape/cvws.shp"):
	...
	for tif in tifs:
		...
		if not os.path.exists(outfn): # Dont write if already there 
			cmd = '''gdalwarp -cutline {} -crop_to_cutline {} {}'''.format(shapefile,tif, os.path.join(dst_dir,outfn))
			os.system(cmd)
...
def main():
	...
	tar_dir = os.path.join(os.getcwd(),'SNODAS_tar')
	gz_dir = os.path.join(os.getcwd(),'SNODAS_tar_gz')
	tif_dir = os.path.join(os.getcwd(),'SNODAS_tifs')
	out_dir = os.path.join(os.getcwd(),'SNODAS_CA_processed')
